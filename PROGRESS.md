...
- **Background Visibility Enhancement**: Increased background image opacity to 90% (`opacity-[0.9]`) globally and reduced the overlying dark mask to 10% (`bg-[#2f2940]/10`) in [BaseLayout.astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/layouts/BaseLayout.astro) to make high-resolution parallax graphics clearly visible on all pages.
- **Contact Information Update**: Updated the company email to `Info@viyuktigroup.com`, mobile number to `+91 7796563374` (and the WhatsApp CTA link), and corporate address to `Punawale, Pune` globally in [siteData.json](file:///C:/Users/bhara/OneDrive/Desktop/client/src/data/siteData.json) and [index.astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/pages/index.astro).
- **About Us Hero Redesign & Scroll Scrubbing**: Converted the About Us page ([about-us.astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/pages/about-us.astro)) Hero section into a 2-column grid (`max-w-7xl`). Set up the left-aligned hero texts on the left and the new graphic asset (`why exist.png`) on the right. Configured GSAP ScrollTrigger matches for scroll scrubbing animations (sliding on desktop, static/centered on mobile).
- **Service Cards Visual Scale-Up**: Increased the image container height in the homepage services slider from `h-56` to `h-64` and the overall card height from `h-[460px]` to `h-[490px]`, making the sliding cards' sector images more prominent and visually striking.
- **Services Hero Redesign & Scroll Scrubbing**: Converted the services detail page ([[slug].astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/pages/services/%5Bslug%5D.astro)) hero section into a spacious 2-column layout (`max-w-7xl`). Configured it to map and display the new sector-specific images (`recruitment and staffing1.jpeg`, `digital marketing 1.jpg`, `it dev and support 1.jpeg`, `realestate 1.jpeg`, `corporate gifting 1.jpeg`, `solar plant 1.jpeg`) on the right, and the left-aligned hero text on the left. Implemented a scroll-linked GSAP scrub parallax animation (sliding on desktop, static/centered on mobile).
- **Static Badges Override**: Removed the `gsap-reveal` sliding animation classes from the homepage domain name badge (`viyuktigroup.com`) and the service page category badges (e.g., `Recruitment & Staffing`) to keep them completely static and anchored in place.
- **Why ViYukti Exists Layout & Scroll Scrubbing**: Reconstructed the "Why ViYukti Exists" section in [about-us.astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/pages/about-us.astro) into a 2-column layout (`max-w-7xl`) with the text left-aligned and the page asset (`about us.jpg`) displayed on the right as a clean, unboxed image. Configured a scroll-linked GSAP scrub animation that matches the Quote section (desktop sliding, mobile centered).
- **Leadership Quote Mobile Alignment Fix**: Updated [index.astro](file:///C:/Users/bhara/OneDrive/Desktop/client/src/pages/index.astro) quote animation to disable horizontal X translations on mobile screens (`max-width: 767px`), keeping the stacked quote text and image perfectly stable and centered.
